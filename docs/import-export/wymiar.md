...
W zakresie wymiany danych o bieżącym stanie należności podatkowych program `PodatkiKS` współpracuje z systemem naliczającym. Informacje o otrzymanych paczkach danych są wyświetlane każdorazowo przy wejściu do programu lub poprzez wybór z menu `Import/Eksport` opcji `Przyjęcie danych z wymiaru`.

![Import wymiaru](screeny\imp_wymiar.png)

W tabeli wyświetlą się informacje o szczegółach paczki. 

Przesłane dane można przeglądać po wyborze przycisku `Pozycje`. W wyświetlonej tabeli dostępne są informacje o ratach należności podatkowych przesłanych z systemu wymiarowego. 
...
Przycisk `Importuj zbiorczo` umożliwia import wszystkich wyświetlonych paczek.
